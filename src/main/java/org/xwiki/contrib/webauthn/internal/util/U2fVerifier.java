...
/**
 * Check if the challenge is signed correctly
 */
public class U2fVerifier
{
    public static boolean verify(AppId appId, RegistrationRequest request, U2fRegistrationResponse response)
        throws CertificateException, IOException, Base64UrlException
    {
        final ByteArray appIdHash = CryptoHelper.sha256(appId.getId());

        final ByteArray clientDataHash =
            CryptoHelper.sha256(response.getCredential().getU2fResponse().getClientDataJSON());

        final JsonNode clientData =
            JacksonCodecs.json().readTree(response.getCredential().getU2fResponse().getClientDataJSON().getBytes());
        ...
        ExceptionUtil.assure(
            request
                ...
                .getChallenge()
                .equals(ByteArray.fromBase64Url(challengeBase64)), "Wrong challenge.");
        ...
        return new U2fRawRegistrationResponse(
